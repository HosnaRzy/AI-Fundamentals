...
def bfs(state):
    ...
    while frontier:
        ...
        if state.is_goal_state():
            return actions
        key = (state.get_enemy_position, state.get_enemy_cycle)
        if key not in visited_counts:
            visited_counts[key] = 0
        if visited_counts[key] >= 2:
            continue
...
def ids(initial_state, start_depth=0, max_depth=20):
